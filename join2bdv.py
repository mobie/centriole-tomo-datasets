...
indir = '/g/schwab/koehrer/EMPIAR/volumes'
suffix = sys.argv[1]

# indir = os.path.join(indir,suffix)

# fileslist = os.path.join('/g/schwab/Tobias/MoBIE/',suffix+'_joinfiles.txt')
joinlist = glob.glob(os.path.join(indir, suffix.split('_')[0], suffix+'*'))

print('Found ' + str(len(joinlist)) + ' files for type ' + suffix + '.')

outdir = '/g/emcf/schorb/code/centrioles-tomo-datasets/data'
